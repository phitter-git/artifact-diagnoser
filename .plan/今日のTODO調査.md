...
#### 3. **再構築シミュレーションのカード順番調整** ✅ 完了
- **内容**: 操作するカードは常に最上部に表示する
- **完了状態**:
  - 結果カードが存在する場合は最上部に表示
  - 操作カードは結果がない場合のみ表示
  - 順序: ① 結果カード（あれば） → ② 操作カード（結果がなければ） → ③ 再構築種別選択 → ④ 希望サブオプション選択

---

#### 6. **再構築シミュレーション結果の枠外余白調整** ✅ 完了
- **内容**: モバイルから見た時、横幅を有効活用する
- **変更内容**:
  - `SingleChildScrollView`の`padding`を16.0から12.0に変更
  - より広い表示領域を確保
...
#### 4. **スコア計算対象変更時、再構築シミュレーションの状態維持** ✅ 完了
- **内容**: スコア計算対象を変更しても、サブオプション選択や再構築種別選択を保持する
- **完了状態**:
  - `AutomaticKeepAliveClientMixin`を使用してタブ切り替え時も状態を保持
  - スコア計算対象変更時は結果のみ再計算、入力は保持
  - タブ遷移しても再初期化されない
...
4. ✅ **TODO 3**: カード順番調整
5. ✅ **TODO 6**: 余白調整（16.0 → 12.0）
6. **TODO 7**: 結果にアイコン・メインステータス表記追加
